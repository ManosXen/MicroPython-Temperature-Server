...
def averageDailyTemp():
    global currentDayLog
    global daily_log_counter
    print(currentDayLog)
    ...
    for temp in currentDayLog:
        if temp==-300:
            break
        sum+=temp
    return round(sum/daily_log_counter, 2)

def temperature_logging(dummy):  #Dummy parameter is used for Timer to work
    prevMeasurement=rtc.datetime()
    prevMeasurementDay=[prevMeasurement[2], prevMeasurement[1], prevMeasurement[0]]
    try: 
        global currentDayLog, daily_log_counter, lastDaysLog, currentDay, maximumTemp
        ...
        currentDayLog[daily_log_counter]=current_temp
        daily_log_counter+=1
